...
!function($){
	var win=$(window)
		...
		,Core=window.FJSCore
		,$Core=$(Core)
		,stateshistory=Core.stateshistory=[]
	
	window.FJSCore.meth={}
	
	window.FJSCore.meth.back=function(){
		Core.stateshistory.pop()
		doc.trigger('changeLocation',Core.stateshistory.pop())
	}
	...
	doc
		.on('changeLocation',function(e,d){
			...
			if(stateshistory.slice(-1)!==Core.state)
				stateshistory.push(Core.state)
			...
			th
				...
				.on('changeState',function(e,d){					
					data.elements.each(function(){
						var th=$(this)
							,a=$('a',th)						
						if(a.attr('href').replace(FJSCore.basepath,'')===d)
...
!function($){
	$(function switcher(){
		...
		FJSCore.internalIds=FJSCore.internalIds.map(function(){return $(this).data('id')})
